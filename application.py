...
stop_ = (1) + (start_ -1)  # stop value for global repetition of the algorithm, (.) = number of global repetitions
...
nSubs_low = 3  # lower bound of number of communities
nSubs_up = 3  # upper bound of number of communities
...
n_steps = 50  # steps of Gibbs iterations  # *** 200
...
rep_forPost = 10  # number of repetitions/sequences of the Gibbs sampling for calculating the posterior distribution  # *** 50
...
rep_end = 40  # end value for number of repetitions in the Gibbs sampling step  # *** 100
...
for nSubs_ in range(nSubs_low, nSubs_up+1):
    ...
    for glob_ind in range(start_,stop_+1):
        ...
        if initGraphonEst:
            ...
            estGraphon0 = estGraphonData0.GraphonEstBySpline(k=1, nSubs=nSubs_, nKnots=nKnots, splitPos=splitPos0, useOneBasis=useOneBasis, tau=None, tau_sep=None, \
                                                             optForAIC=True, lambdaMin=None, lambdaMax=None, calcAIC=False, lambda_=None, \
        ...
        if initPostDistr:
            if rep_forPost == 0:
                ...
                sample0.gibbs(steps=n_steps,proposal=proposal,rep=rep_forPost,sigma_prop=sigma_prop, splitPos=None, returnAllGibbs=False, averageType=averageType, updateGraph=False, use_stdVals=None, printWarn=True)
                ...
                # plot network with initial ordering
                graph0.showNet(splitPos=estGraphon0.splitPos, make_show=make_show, savefig=savefig, file_=dirExt + 'network_1.png')
                if simulate:
        ...
        EM_obj = iterateEM(sortG=graph0,
                           k=k, nSubs=nSubs_, nKnots=nKnots, useOneBasis=useOneBasis, canonical=canonical, est_initSplitPos=est_initSplitPos, adjustSubs=adjustSubs, adjustQuantiles=adjustQuantiles,
        ...
        result_ = np.array([nSubs_, nTry, EM_obj.estGraphon.AIC, EM_obj.estGraphon.AICc, EM_obj.estGraphon.AICL1, EM_obj.estGraphon.AICL2, EM_obj.estGraphon.AICL2b, EM_obj.estGraphon.ICL, EM_obj.estGraphon.optCrit, EM_obj.estGraphon.optValue, EM_obj.sortG.logLik(graphon = EM_obj.estGraphon), EM_obj.estGraphon.logLik, EM_obj.estGraphon.df_lambda, (np.round(EM_obj.lambda_, 4).__str__() if np.isscalar(EM_obj.lambda_) else ' '.join([np.round(EM_obj.lambda_, 1)[i].__str__() for i in range(EM_obj.lambda_.shape[0])]).replace('\n', ''))])
        print(result_)
        ...
        if not os.path.isfile(fname):
            with open(fname, 'a') as fd:
                fd.write('nSubs; nTry; AIC; AICc; AICL1; AICL2; AICL2b; ICL; optCrit; optValue; logLik1; logLik2; df_lambda; seed; seed2; estMethod; initGraphonEst; initCanonical; initPostDistr; trueInit; N; k; nKnots; splits; canonical; adjustSubs; adjustQuantiles; n_steps; sigma_prop; averageType; use_stdVals; rep_forPost; n_iter; rep_start; rep_end; it_rep_grow; lambda_start; lambda_skip1; lambda_lim1; lambda_skip2; lambda_lim2; lambda_last_m; lambda_; \n')
        with open(fname, 'a') as fd:
            fd.write(';'.join(result_[:13].astype(str)) + ';' + seed_.__str__() + ';' + seed2_.__str__() + '; ' + estMethod.__str__() + '; ' + initGraphonEst.__str__() + '; ' + initCanonical.__str__() + '; ' + initPostDistr.__str__() + '; ' + trueInit.__str__() + '; ' + N.__str__() + '; ' + k.__str__() + '; ' + EM_obj.estGraphon.nKnots.__str__() + ';' + np.array2string(EM_obj.estGraphon.splitPos, precision = 3, max_line_width = 100000, separator = ',') + '; ' + canonical.__str__() + '; ' + adjustSubs.__str__() + '; ' + adjustQuantiles.__str__() + '; ' + n_steps.__str__() + '; ' + sigma_prop.__str__() + '; ' + averageType + '; ' + use_stdVals.__str__() + '; ' +
...
#entry_i[2] = AIC, entry_i[3] = AICc, entry_i[4] = AICL1, entry_i[5] = AICL2, entry_i[6] = ICL, entry_i[7] = logLik, entry_i[9] = optValue
...
if not os.path.isfile(fname):
    with open(fname, 'a') as fd:
        fd.write('nSubs; nTry; AIC; AICc; AICL1; AICL2; AICL2b; ICL; optCrit; optValue; logLik1; logLik2; df_lambda; lambda_; \n')
with open(fname, 'a') as fd:
